...
class Automata 
{
	...
	protected boolean estDespInval;
			...
			if(!An.regresaOper().equalsIgnoreCase("NULL"))
			{
				if(An.regresaOper().matches("\"[^\"]*\""))
				{
					if(An.regresaOper().contains("''"))
						An.ingresarOperando(An.regresaOper().replace("''","\\\\")); //se remplaza el separador especial para el escape de diagonal invertida
					if(An.regresaOper().contains("|"))
						An.ingresarOperando(An.regresaOper().replace("|"," ")); //se remplaza el separador especial por el espacio
					if(An.regresaOper().contains("\\°"))
						An.ingresarOperando(An.regresaOper().replace("\\°","\\\"")); //se remplaza el separador especial por el escape de comilla
					if(An.regresaOper().contains("¬"))
						An.ingresarOperando(An.regresaOper().replace("¬",";")); //se remplaza el separador especial por el simbolo de comentario
					
					String cad = An.regresaOper().substring(1, An.regresaOper().length()-1);
					
					if(cad.contains("\\\\"))
						cad = cad.replace("\\\\","\\"); //eliminacion de la diagonal invertida de escape
					if(cad.contains("\\\""))
						cad = cad.replace("\\\"","\""); //eliminacion de la diagonal invertida de escape
