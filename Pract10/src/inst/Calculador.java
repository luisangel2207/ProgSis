...
class Calculador 
{
					...
					&& direcc.equalsIgnoreCase(listadeInst[i].regresaInst())) //Si coinciden Codop y Direcc 
			{
				...
				if(listadeInst[i].regresaBPC() != 0) //Si existen Bytes por Calcular
				{
					...
					case 2:
						if(oper.matches("[a-zA-z]+[a-zA-z0-9_]*"))
						{
							int valoper = Integer.parseInt(arbolDeEtq.get(oper),16); //rr
							int valcontloc = Integer.parseInt(contloc,16) + (2 + codmaq.length()/2); //ContLocSig
							valoper -= valcontloc; // aplicamos formula ff - ContLocSig
							if(valoper >= -32768 && valoper <= 65535) //si esta dentro del rango
							{
								codhex = Integer.toHexString(valoper).toUpperCase(); //se pasa a formato HEX
								if(valoper >= 0) //si es positivo
								{
									switch(codhex.length())
									{
									case 1:
										...
										break;
									case 2:
										codhex = "00" + codhex; //se complementa con ceros si es necesario
										break;
									case 3:
										codhex = "0" + codhex;
										break;
									...
									codhex = codhex.substring(4); //se toman los 2 bytes menos significativo del Comp2
								ingresaEstEncontrado(true);
							}
							else
								return "XXXX"; //error de rango detectado, regresa simbolo de error
	...
	public String obtenerCodMaqFCC(Vector<String> elemento)
	{
		...
		codhex = elemento.toString(); //transformamos nuestro Vector en una cadena
		int ind = codhex.indexOf("\""); //buscamos la primera ocurrencia de unas comillas
		codhex = codhex.substring(ind); //hacemos una subcadena a partir de las primeras comillas
		codhex = codhex.substring(1, codhex.lastIndexOf("\"")); //eliminamos las comillas de la cadena
		codhex = codhex.replace(",",""); //eliminamos las comas (que nos entrega el Vector por defecto) de la cadena
		
		if(codhex.contains("\\\\")) 
			codhex = codhex.replace("\\\\","\\"); //eliminacion de la diagonal invertida de escape
		if(codhex.contains("\\\""))
			codhex = codhex.replace("\\\"","\""); //eliminacion de la diagonal invertida de escape
		for(char letra : codhex.toCharArray()) //se itera la cadena caracter por caracter
		{
			short num = (short)letra; //se hace el casting para obtener el valor ASCII de la letra
			codmaq += Integer.toHexString(num).toUpperCase(); //se va concatenando el valor en formato HEX de cada letra
		}
