...
public class Analizador 
{
	...
	public void errorDetectado(boolean estado)
	{algunerror = estado;}
	public boolean existeAlgunError()
	{return algunerror;}
	...
	public String describirError(byte error, String palabra)
	{
		...
		switch(error)
		{
			...
			case 26:
				...
				break;
			case 27:
				tipo = "Error, Se detecto al menos un error en el Archivo Inst" + palabra;
				break;
		}
		return tipo;
	}
	...
	public static void main(String[] args) throws IOException
	{
		...
		try
		{
			...
			if(!Au.regresaEstInstConErr() || !An.existeAlgunError()) //Si no tiene errores el Archivo Inst
			{
			...
			//Generar Archivo S19
			if(!An.existeAlgunError()) //Si ningun error se genero en todas las validaciones
			{
				nombre1 = nombre1.replace(".tds", ".s19"); //se cambia la extension del nombre a .s19
				Verificador Ve = new Verificador(); 
				Archivo ArcS19 = new Archivo(ruta1,nombre1,(byte)0); //Se crea un Objeto Archivo S19
				Formato Fo = new Formato(); //Se crea un Objeto Formato
				
				if(ArcS19.existeArchivo()) //Si existe el Archivo .s19
					ArcS19.borrarArchivo(); //borramos el archivo
				
				Fo.generarRegS0(nombre,ruta1,ArcS19); //Generamos S0
				Fo.generarRegS1(An,Ve,ArcIns,ArcS19); //Generamos S1
				Fo.generarRegS9(ArcS19); //generamos S9
			}
			else
				ArcErr.errorGeneralS19(An); //Algun error en el archivo Inst
		}// fin try
		catch (IOException e) 
		{	
			e.printStackTrace();
