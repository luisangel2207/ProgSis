...
class Direccionamiento 
{
	...
	public void conversorDeBase(Analizador An,Automata Au,Archivo ArcErr,Direccionamiento Di,short cont)
	{
		...
		{
		case '$':
			baseHexadecimal(An.regresaOper().substring(1),An,Au,ArcErr,Di,cont);  //Verificamos valor Hexadecimal
			break;
		case '@':
			baseOctal(An.regresaOper().substring(1),An,Au,ArcErr,Di,cont);	//Verificamos rango y valor Octal
			break;
		case '%':
			baseBinaria(An.regresaOper().substring(1),An,Au,ArcErr,Di,cont); //Verificamos rango y valor Binario
			break;
		case '-':
		case '0':
		case '1':
		...
		case '8':
		case '9':
			baseDecimal(An.regresaOper(),An,Au,ArcErr,Di,cont); //Verificamos rango y valor Decimal
			break;
		default:
			ArcErr.errorSimboloDeBase(An,cont);
			Au.ingresaEstErrNum(true);
