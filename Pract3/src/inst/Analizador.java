...
public class Analizador 
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		try
		{
			...
			encabezado = "LINEA	ETQ	CODOP	OPER	MODDIR";	//Encabezado del Archivo .inst
			...
			while((linea = ArcAsm.lineaActual()) != null && termina == false)
			{
				...
				if(!Au.regresaEstEnd())		//Si el Codop aun no es la directica END
				{
					if(band && Au.regresaEstSinErr() && !Au.regresaEstErrDet())	//instruccion valida
					{
						...
						{
							Au.estadoValidaInst(In, An, Au, Di, listadeInst, ArbolDeInst, ArcIns, ArcErr, cont); //entra al Automata para validar Codop-Operando
						}
